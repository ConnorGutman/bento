  ...
  render() {
    ...
    if (this.state.stories.length === 0 && this.state.ids.length === 0 && this.getPageNumber() > 0) {
      ...
      for (var i = page.startIndex; i < page.endIndex; i++) {
        dummyItems.push(
          <div key={i} className="row post ListItem ListItem--loading" style={{marginBottom: SettingsStore.listSpacing}}>
            <Spinner/>
          </div>
        )
      }
      return <div className="Items Items--loading">
        <div className="container main-content Items__list" start={page.startIndex + 1}>{dummyItems}</div>
