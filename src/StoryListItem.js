...
var StoryListItem = React.createClass({
  ...
  /**
   * Initialise listening to updates for the item with the given id
   */
  initLiveItem(props) {
  ...
  render() {
    ...
    if (!this.state.item || !this.state.item.id) {
      return <div className="row post ListItem ListItem--loading" style={{marginBottom: SettingsStore.listSpacing}}>
        <Spinner/>
      </div>
    }
